<template>
  <div class="bg-white/20 backdrop-blur-md rounded-xl p-6 shadow-lg transition-all duration-300 hover:shadow-xl hover:bg-white/30">
    ...
    <div class="space-y-6">
      <div v-for="(payment, index) in paymentSchedule" :key="index" class="relative">
        <div class="flex items-center mb-2">
          ...
          <div class="flex-grow">
            ...
            <div class="text-xs text-white/70">Termijn {{ index + 1 }}</div>
          </div>
        ...
        <div v-if="index < paymentSchedule.length - 1" class="absolute left-5 top-10 bottom-0 w-0.5 bg-white/30"></div>
      </div>
    </div>
  </div>
</template>
